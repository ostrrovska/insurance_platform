...
Lab5 extension: also handles PolicyActivated and PolicyCancelled events
so we can observe saga outcomes in the notification log.
...
class RabbitMQConsumer:
    async def _setup(
        ...
        # Bind to ALL routing keys from both exchanges (# wildcard = any suffix)
        # This captures: customers.created, policies.created, policies.activated, policies.cancelled
    ...
    async def _handle_message(self, message: aio_pika.abc.AbstractIncomingMessage) -> None:
        async with message.process(requeue=False):
            ...
            async with AsyncSessionLocal() as session:
                # Idempotency check — insert with unique event_id, skip if duplicate
                ...
                # Dispatch to domain handler if one is registered
